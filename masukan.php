...
if (isset($_GET['id_masukan'])) {
    $id_masukan = $_GET['id_masukan'];
    $query = "SELECT * FROM masukan WHERE id_masukan = '$id_masukan'";
    $result_set = $conn->query($query);
    $masukan = $result_set->fetch_assoc();

    $_SESSION['masukan_id'] = $masukan['id_masukan'];
} else {
    $masukan = null;
}

if (isset($_POST['hapus'])) {
    $hapus_id_masukan = $_POST['hapus_id_masukan'];
    // Lakukan proses penghapusan data masukan sesuai dengan $hapus_id_masukan
    // ...

    // Hapus session masukan_id jika data masukan dihapus
    if ($_SESSION['masukan_id'] == $hapus_id_masukan) {
        unset($_SESSION['masukan_id']);
    }
                            ...
                            <table id="datatablesSimple">
                                ...
                                <tbody>
                                    ...
                                    while ($row = $result_set->fetch_assoc()) {
                                        ...
                                        if ($row['kondisi'] != 'Show') {
                                            echo ('<form method="post" action="update_masukan.php">');
                                        ...
                                        echo ('<a href="hapus_masukan.php?id_masukan=' . $row['id_masukan'] . '" class="btn btn-danger">Hapus</a>');
                                        echo ('</div>');
                                        echo ('</td>');
                                        echo ('</tr>');
                                    }
